...
// 'agents' is NOT injectable here for childCtx: the agent-loop calls our
// `setup(childCtx, child)` with childCtx = the CHILD agent's ctx, whose scope
// exposes systemPrompt/tools services but NOT the `agent` accessor (same as
// stock dsh-subagent, which reads `child.session` from the second argument —
// see dsh-subagent/lib/index.js:1077/542). Use the `child` Agent handle.
...
export function apply(ctx, config = {}) {
  ...
  const makeProvider = (pname, inheritsParentContext) => ({
    ...
    // agentOptions is true: the tool resolves per-call model/provider/maxTokens
    // through resolveChildAgentOptions — without this flag the service rejects
    // any request carrying agentOptions (UNSUPPORTED_CAPABILITY).
    ...
    async start(request) {
      ...
      const setup = async (childCtx, child) => {
        // child = the published Agent handle (second setup argument, same as
        // stock dsh-subagent:1077). The `agent` accessor is NOT visible on the
        // child ctx scope (see inject note) — use the handle directly.
      ...
      return drivePresetRun(
        await parent.ctx.agents.create({
          ...
          // last turn/end as its own completion). isSeeded must be a boolean
          // (0.1.5-rc.1: childSessionMeta's third arg is `isSeeded`, not the
          // boundary number); inheritedEventCount carries the same prefix
          // length so the session boundary knows where the inherited events end.
          meta: { ...childSessionMeta(parent, childDepth, boundary > 0), agentPreset: targetPreset },
          ...(boundary > 0 ? { seed, inheritedEventCount: boundary } : {}),
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if ((engine !== 'claude' && engine !== 'codex' && engine !== 'codebuddy') && (agentOptions.model !== undefined || agentOptions.provider !== undefined)) {
        ...
        // Live parent route (0.1.5-rc.1 helper): the session's request header
        // wins over the agent's creation options, so a mid-session model switch
        // is what the child actually inherits.
        const parentRoute = parentAgentOptionsForDelegation(parent)
        const provider = agentOptions.provider ?? parentRoute.provider
        const model = agentOptions.model ?? parentRoute.model
        if (llm !== undefined && provider !== undefined && model !== undefined && typeof llm.resolveModelInfo === 'function') {
          try {
            ...
            const shown = agentOptions.model !== undefined ? agentOptions.model : `${parentRoute.model ?? '(parent model)'}`
            throw new Error(
      ...
      if (args.continuable) {
        if (externalEngine) {
          ...
          // registerExternalContinuation lives in the PATCHED dsh-subagent fork.
          // Resolve it DYNAMICALLY (same specifier as the static import above —
          // this is a module-cache hit) so the plugin still LOADS on an
          // unpatched runtime: external-engine continuable degrades to a
          // warning instead of a module-load SyntaxError.
          try {
            const subagentMod = await import('@deepseek-ai/dsh-subagent')
            if (typeof subagentMod.registerExternalContinuation === 'function') {
              subagentMod.registerExternalContinuation(prepared.sessionId, prepared.resume)
            } else {
              ctx.logger?.warn?.(`${toolName}: external-engine continuable needs the patched dsh-subagent fork (registerExternalContinuation missing) — subagent started WITHOUT send_message resume`)
            }
          } catch (error) {
            ctx.logger?.warn?.(`${toolName}: external-continuation registry unavailable: ${String(error?.message ?? error)}`)
          }
          return {
            kind: 'continuable',
